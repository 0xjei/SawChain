...
const checkParameterValue = (parameter, value, dataType) => {
    ...
    if (dataType === TypeData.DataType.NUMBER) {
        ...
        if (value.numberValue > parameter.maxValue)
            reject(`The value ${value.numberValue} is greater than the Parameter maximum value constraint ${parameter.maxValue}`)
    ...
    // String data type.
        ...
        if (value.stringValue.length > parameter.maxLength)
            reject(`The value length ${value.stringValue.length} is greater than the Parameter maximum length constraint ${parameter.maxLength}`)
    }
}
...
async function createDescriptionEvent(
    ...
    if (batch.length === 0 && field.length === 0)
        reject(`No Batch or Field specified`)

    // Validation: Either Batch and Field specified.
    if (batch.length > 0 && field.length > 0)
        reject(`Either Batch and Field specified`)
    ...
    if (eventTypeState.typology !== EventType.Typology.DESCRIPTION)
        reject(`The Event Type ${eventType} is not a description Event Type`)
    ...
    if (values.length !== eventTypeState.parameters.filter(p => p.required).length)
        reject(`No values specified for required Parameters`)
    ...
    for (const parameter of eventTypeState.parameters) {
        let eventParameterTypeAddress = parameter.eventParameterTypeAddress

        let state = await context.getState([eventParameterTypeAddress])

        const eventParameterTypeState = EventParameterType.decode(state[eventParameterTypeAddress])

        values.forEach(value => {
            if (eventParameterTypeAddress === value.parameterType)
                checkParameterValue(parameter, value, eventParameterTypeState.dataType)
        })
    }
...
async function createTransformationEvent(
    ...
    if (batches.length === 0 && fields.length === 0)
        reject(`No Batches or Fields list specified`)

    // Validation: Either Batches and Fields lists specified.
    if (batches.length > 0 && fields.length > 0)
        reject(`Either Batches and Fields lists specified`)

    // Validation: No quantities list specified.
    if (quantities.length === 0)
        reject(`No quantities list specified`)

    // Validation: No output batch id specified.
    if (!outputBatchId)
        reject(`No output batch id specified`)
    ...
    if (eventTypeState.typology !== EventType.Typology.TRANSFORMATION)
        reject(`The Event Type ${eventType} is not a transformation Event Type`)
    ...
    if (fields.length > 0) {
        ...
        if (fieldsState.some(field => field.product !== fieldsState[fieldsState.length - 1].product))
            reject(`A field doesn't match other Field's product Product Type`)
    ...
    if (batches.length > 0) {
        ...
        if (batchesState.some(batch => batch.product !== batchesState[batchesState.length - 1].product))
            reject(`A batch doesn't match other Batch's product Product Type`)
    ...
    if (quantities.some(qnt => qnt <= 0))
        reject(`At least one quantity is not greater than zero`)

    if (fields.length > 0) {
        // Validation: Quantities length doesn't match fields length.
        if (quantities.length !== fields.length)
            reject(`Quantities length doesn\'t match fields or batches length`)

        // Validation: A quantity is greater than current Field quantity.
        quantities.forEach((quantity, index) => {
            if (fieldsState[index].quantity - quantity < 0)
                reject(`A quantity is greater than current Field quantity ${quantity}`)
        })
    }

    if (batches.length > 0) {
        // Validation: Quantities length doesn't match batches length.
        if (quantities.length !== batches.length)
            reject(`Quantities length doesn\'t match batches or batches length`)

        // Validation: A quantity is greater than current Batch quantity.
        quantities.forEach((quantity, index) => {
            if (batchesState[index].quantity - quantity < 0)
                reject(`A quantity is greater than current Batch quantity ${quantity}`)
        })
    }

    /// Validation: Output Batch id is already used for another Company Batch.
    if (companyState.batches.some(batch => batch === outputBatchAddress))
        reject(`Output batch id ${outputBatchId} is already used for another Company Batch`)
